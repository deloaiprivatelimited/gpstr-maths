...
def ffmpeg_segment(slide: str, audio: str, out: str, duration: float):
    """Image + audio → mp4 segment"""
    subprocess.run([
        "ffmpeg", "-y",
        "-loop", "1", "-i", slide,
        "-i", audio,
...
def ffmpeg_concat(segment_paths: list[Path], out: Path):
...
def ffmpeg_with_intro_end(content_video: Path, out: Path):
    subprocess.run([
        "ffmpeg", "-y",
        "-i", INTRO_VIDEO,
        "-i", str(content_video),
...
def assemble_video(plan: list[dict], timeline: list[dict],
                   slides_dir: Path, output_path: Path):

    # build label → timeline entry map
    tl_map = {t["label"]: t for t in timeline}
    ...
    for entry in plan:
        ...
        tl    = tl_map.get(label)
        if not tl:
            print(f"  ⚠ No timeline entry for {label}, skipping")
            continue

        audio_path = Path(tl["file"])
        ...
        if not slide_path.exists():
            print(f"  ⚠ Slide missing: {slide_path}")
            continue

        seg_out = slides_dir / f"seg_{label}.mp4"
        if not seg_out.exists():
            ffmpeg_segment(str(slide_path), str(audio_path),
                           str(seg_out), tl["duration"])
        segments.append(seg_out)

    if not segments:
        print("  ⚠ No segments, skipping assembly")
        return

    content_video = slides_dir / "module_content.mp4"
    ffmpeg_concat(segments, content_video)
...
def process_module(args):
    ...
    # run async slide generation in subprocess context
    plan = asyncio.run(generate_slides(chunk_data, slides_dir))

    assemble_video(plan, timeline, slides_dir, output_path)
...
def collect_jobs() -> list[tuple]:
    ...
    for chunk_file in sorted(CHUNKS_DIR.rglob("*.json")):
        ...
        # audio/{chapter}/{module_id}/timeline.json
        timeline_file = AUDIO_DIR / relative.parent / module_id / "timeline.json"

        # videos/{chapter}/{module_id}.mp4
